...
6. For the current watch-party prototype, use `bambu_generate_world_cup_figurines`.
7. For export, use `bambu_openscad_export_plan`; do not guess the OpenSCAD command.
8. For slicing, use `bambu_slice_plan`; it should use detected Bambu Studio or OrcaSlicer executables.
9. For the full safe prototype path, use `bambu_build_world_cup_prototype`; it generates SCAD, STL, and sliced 3MF but does not start the printer.
10. After a physical print, call `bambu_record_print_result` with outcome, measurements, material state, failure mode, and next revision notes.
11. Do not start print jobs. Stop at a plan and require manual approval before printer contact.
